...
export default function LevelCard({ level, xp, xpTo, steps, reached }: Props) {
  ...
  React.useEffect(() => {
    if (!row) return;
    grow.value = 0;
    grow.value = withDelay(180, withTiming(1, { duration: 620, easing: Easing.out(Easing.cubic) }));
  }, [row, fillTo, grow]);
      ...
      <View style={styles.progress} onLayout={(e) => setRow(e.nativeEvent.layout.width)}>
        <View style={styles.trackRow}>
          ...
          {steps.map((step, i) => {
            ...
            return (
              <View key={step} style={[styles.badge, done ? styles.badgeDone : styles.badgeLocked]}>
                {done ? (
